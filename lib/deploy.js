...
module.exports = Object.create({
  ...
  withBumpType: function (options0, options1) {
    ...
    const
      ...
      REF_SHA_CMD    = 'git rev-parse --verify HEAD',
      CURRENT_BRANCH = ChildProcess.execSync(REF_HEAD_CMD)
                         .toString().replace('\n',''),
      CURRENT_SHA    = ChildProcess.execSync(REF_SHA_CMD)
                         .toString().replace('\n',''),
    ...
    function pushAndMaybePublish () {
      return git.push(remote, CURRENT_BRANCH, { args: args }, function (error) {
        if (error) {
          ...
          log('`git push` failed, reverting your unchanged files to their state prior to running Unleash')
          ChildProcess.execSync(`git reset --mixed ${CURRENT_SHA}`)
          throw error
        }
